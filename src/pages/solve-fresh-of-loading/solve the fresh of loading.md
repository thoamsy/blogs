...
生活很美好吧，umh？不过呢，这让整个交互体验变得不那么「人性化」了。因为不论任何时候，网速怎么样，都会先生硬地加载一个 loading UI，然后在接口返回的时候，_匆匆完事_。
...
1. 出现 loading，会让用户觉得变卡了（设想一下每次微信聊天界面都有一个 loading）
2. 闪烁的体验让人很分裂
...
不过这个例子其实和我们要讨论的不一样，上面的例子是关于 Code Splitting，我们主要讨论的是 Ajax 请求的。
...
根据[官方的说法](https://reactjs.org/blog/2018/11/27/react-16-roadmap.html#tldr)，等到 `ConcurrentMode` 的稳定，至少也要到 2019 年的 Q2，而客户端级别的 `Suspense` 要到 Q3。另外一些强大的库，比如 `rxjs`，似乎也能轻松做到我们的需求，但是因为门槛高，用户基数不大，不在讨论范围内。
...
timeout 的实现使用 reject 而不是 resolve 可以让代码更加简洁。因为进入 /fulfilled/ 的情况只有一种，就是在 ms 之内请服务器响应并返回了。而在 rejected 状态下，就是超时的逻辑（_注意 ⚠️，这里不考虑请求异常的情况_）
...
嗯，看上去解决了上面的问题。不过它解决问题的方法是通过 _再发一次请求_，相当于没有解决问题。
但其实我们已经接近到那隧道尽头的光了。

如果 `const rep = await fetch(url)` 这句话不会重新发送请求，而仅仅只是接着第一次调用 `fetch(url)` 的场景，承载着那个 Promise，是不是就解决我们的问题了？

是的，我们只需要将 `fetch(url)` 保存下来，就可以做到了。
...
在整个实现中，我们忽略了一个问题，那就是 request 也有 _throw 异常_ 的情况，那么一个关键的问题就需要在 catch 中区分到底是 超时还是服务器异常导致的问题。而为了区分这个，最好的方式就是 Symbol。
可以将 timeout 改为：
