...
PS：如果你想去体验的话，可能会失望。现在 react-cache 和 16.8.4 放在一起，根本跑不起来 🌚

## 总结

通过上面四种逐渐改进的开发模式 Class -\> Class + Context -\> Hooks -\> Suspense 来看，Suspense 是下一个 big thing，它会解决目前 React 下长久存在的一些问题，并在这同时还做到了一定程度上的 UX 改进。从 三十四行代码减少为十行左右，减少的不仅仅是代码行数，还有 React 开发者之间的沟通的距离。
与其同时，它让整个异步代码都变的更加**声明式**：一个组件只需要关注如何渲染 UI，而不同于之前的组件是关注如何使用 props，这是一个思考方式上的改变，也让整个组件的权责更为专注。
